...
public class BackstageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(type==null){
            ...
            List category=FactoryDao.getCategoryDaoInstance().selectAll();
            request.getSession().setAttribute("CategoryList",category);
            try {
                response.sendRedirect("Backstage/type.jsp");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }else if(type.equals("user")){
            List person=FactoryDao.getPersonDaoInstance().selectAllPerson(1);
            request.getSession().setAttribute("PersonList",person);
            try {
                response.sendRedirect("Backstage/deleteUser.jsp");
            } catch (IOException e) {
                e.printStackTrace();
            }
